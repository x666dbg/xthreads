    ...
    <div class="max-w-4xl mx-auto px-4 py-6">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            @if(auth()->user()->unreadNotifications->count() > 0)
                <form action="{{ route('notifications.read-all') }}" method="POST">
                    @csrf
                    <button type="submit" class="text-primary-400 hover:text-primary-300 text-sm font-medium transition-colors">
                        Mark all as read
                    </button>
                </form>
            @endif
        ...
        <div class="space-y-1">
            @forelse($notifications as $notification)
                <div class="bg-dark-800/30 backdrop-blur-sm border border-dark-700/50 rounded-xl p-4 hover:bg-dark-700/30 transition-all duration-200 
